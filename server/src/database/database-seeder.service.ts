  ...
  async onApplicationBootstrap() {
    ...
    await this.dataSource.transaction(async (manager) => {
      const users = await manager.getRepository(UserEntity).save([
        { email: 'admin@shginvest.vn', passwordHash, fullName: 'Nguyen Hoang An', phone: '0901000001', role: UserRole.ADMIN, status: UserStatus.ACTIVE, createdAt: date('2026-01-05T08:00:00Z'), updatedAt: date('2026-04-12T08:00:00Z') },
      ...
      await manager.getRepository(AuditLogEntity).save([
        { userId: userMap.get('admin@shginvest.vn')!.id, action: 'company.verify', entityType: 'company', entityId: companyMap.get('Sunrise Manufacturing')!.id, oldData: { status: 'pending' }, newData: { status: 'verified' }, createdAt: date('2026-04-10T08:30:00Z') },
      ...
      await manager.getRepository(ReportEntity).save([
        { userId: userMap.get('admin@shginvest.vn')!.id, reportType: 'Fraud profile', targetName: 'Nhan vien kho van ca dem', severity: ReportSeverity.HIGH, status: ReportStatus.REVIEWING, description: 'Can kiem tra lai mo ta viec va thong tin lien he.', createdAt: date('2026-04-11T09:00:00Z') },
